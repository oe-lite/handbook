...
variable, and the first package with a `FILES_*` glob pattern matching
a given file gets it. This means that when creating custom packages,
one usually uses the prepend operator ``=+`` to add the new package
name.
